...
2. `yaml2json.py` 读取 YAML，通过 Jinja2 模板（`yaml2json/tmpl`）渲染出完整的 xray JSON 配置
...
- `saveDay` 保存快照时通过 `-reset` 参数原子地读取并清零计数器
...
│   ├── tmpl                   # Jinja2 模板
│   └── requirments.txt        # Python 依赖（pyyaml, jinja2）
...
3. 创建 yaml2json 虚拟环境并安装 Python 依赖（pyyaml、jinja2）
4. 创建 `clash` 系统组
...
# 查看历史记录
xray.py traffic day  myserver    # 年度文件
xray.py traffic hour myserver    # 月度文件
```
...
- 每天 23:59：`xray.py traffic saveDay <name>`
